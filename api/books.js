export default async function handler(req, res) {
    try {
        ...
        console.error(`Error fetching data for range ${req.query.start}-${req.query.end}:`, error.message);
        res.status(500).json({ error: "Failed to fetch data from OpenAPI", details: error.message });
    }
}
